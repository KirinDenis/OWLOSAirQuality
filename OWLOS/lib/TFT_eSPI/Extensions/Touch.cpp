...
** OWLOS change this original method source code
***************************************************************************************/
void TFT_eSPI::calibrateTouch(uint16_t *parameters, uint32_t color_fg, uint32_t color_bg, uint32_t color_screen_bg, uint8_t size)
{
  String TouchMessage = "touch this red rectangle";

  int16_t values[] = {0, 0, 0, 0, 0, 0, 0, 0};
  uint16_t x_tmp, y_tmp;

  fillScreen(color_screen_bg);
  setTextColor(color_fg, color_screen_bg);

  setTextSize(2);
  setCursor(_width / 2 - textWidth("Calibrate touch") / 2, _height / 2 - fontHeight(2) / 2);
  print("Calibrate touch");
  ...
  for (uint8_t i = 0; i < 4; i++)
  {
    ...
    fillRect(0, 0, size + 1, size + 1, color_screen_bg);
    fillRect(0, _height - size - 1, size + 1, size + 1, color_screen_bg);
    fillRect(_width - size - 1, 0, size + 1, size + 1, color_screen_bg);
    fillRect(_width - size - 1, _height - size - 1, size + 1, size + 1, color_screen_bg);
    ...
    case 0: // up left
      ...
      drawLine(size, 0, 0, size, color_fg);

      setCursor(size + size / 4, size + size / 4);
      print(TouchMessage);
      break;
    case 1: // bot left
      fillRect(0, _height - size - 1, size + 1, size + 1, color_bg);
      drawLine(0, _height - size - 1, size, _height, color_fg);
      drawLine(size, _height - size - 1, 0, _height, color_fg);

      setCursor(size + size / 4, size + size / 4);
      setTextColor(color_screen_bg, color_screen_bg);
      print(TouchMessage);

      setCursor(size + size / 4, _height - (size + fontHeight(1)));
      setTextColor(color_fg, color_screen_bg);
      print(TouchMessage);
      break;
    case 2: // up right
      fillRect(_width - size - 1, 0, size + 1, size + 1, color_bg);
      drawLine(_width - size - 1, 0, _width, size, color_fg);
      drawLine(_width, 0, _width - size - 1, size, color_fg);

      setCursor(size + size / 4, _height - (size + fontHeight(1)));
      setTextColor(color_screen_bg, color_screen_bg);
      print(TouchMessage);

      setCursor(_width - (size + size / 4 + textWidth(TouchMessage)), size + size / 4);
      setTextColor(color_fg, color_screen_bg);
      print(TouchMessage);
      break;
    case 3: // bot right
      fillRect(_width - size - 1, _height - size - 1, size + 1, size + 1, color_bg);
      drawLine(_width - size - 1, _height - size - 1, _width, _height, color_fg);
      drawLine(_width, _height - size - 1, _width - size - 1, _height, color_fg);

      setCursor(_width - (size + size / 4 + textWidth(TouchMessage)), size + size / 4);
      setTextColor(color_screen_bg, color_screen_bg);
      print(TouchMessage);

      setCursor(_width - (size + size / 4 + textWidth(TouchMessage)), _height - (size + fontHeight(1)));
      setTextColor(color_fg, color_screen_bg);
      print(TouchMessage);
      break;
    }
